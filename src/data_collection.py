...
def capture_user_images(user_name):
    """
    Captures 5-10 images from the webcam and saves them for the specified user.
    """
    ...
    print(f"Please look at the camera. Capturing {config.NUM_IMAGES_TO_CAPTURE} images...")
    
    # Wait for the camera to warm up
    time.sleep(2)
    
    count = 0
    while count < config.NUM_IMAGES_TO_CAPTURE:
        ret, frame = cap.read()
        if not ret:
        ...
        print(f"Captured {count+1}/{config.NUM_IMAGES_TO_CAPTURE}: {img_path}")
